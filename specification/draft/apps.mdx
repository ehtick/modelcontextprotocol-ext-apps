...
// Request
{
  ...
  params: {
    ...
    mimeType: string,                   // MIME type (e.g. "image/svg+xml", "application/json")
    encoding?: "utf-8" | "base64"      // Content encoding, defaults to "utf-8"
  }
}
...
* Host SHOULD use the `filename` as the suggested filename for the download.
* For `encoding: "base64"`, host MUST decode the content from base64 before creating the file.
* Host MAY reject the download based on security policy, file size limits, or user preferences.
* Host SHOULD sanitize the filename to prevent path traversal.
